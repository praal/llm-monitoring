#!/usr/bin/env python3
"""
Evaluate LLM on multi-simple traces.
...
DATASET_PATH = "data/multi_simple_dataset.json"
RESULTS_DIR = "results-multi-simple"
SAVE_EVERY = 10
...
def eval_single_sample(sample, model, temperature, max_tokens, timeout=None):
...
def main():
    parser = argparse.ArgumentParser(
        description="Evaluate LLM on multi-simple traces"
    )
    ...
    output_path = os.path.join(RESULTS_DIR, f"results_multi_simple_{model_slug}.json")
            ...
            for future in as_completed(futures):
                ...
                if completed_count % SAVE_EVERY == 0:
                    with save_lock:
                        ...
                        metrics = compute_f1(all_samples)
